...
typedef struct _QN_STORAGE
{
    ...
    struct {
        qn_svc_selector_ptr up;
        qn_svc_selector_ptr io;
        qn_svc_selector_ptr rs;
        qn_svc_selector_ptr rsf;
        unsigned int strategy:16;
        unsigned int filter:16;
    } sel;
} qn_storage;
...
{
    if (stor) {
        qn_stor_set_region(stor, NULL, QN_SVC_SEL_LAST_SUCCEEDED_FIRST, QN_SVC_SEL_ANY);
        if (stor->obj_body) qn_json_obj_destroy(stor->obj_body);
        if (stor->arr_body) qn_json_arr_destroy(stor->arr_body);
...
static qn_bool qn_stor_create_service_selector(qn_storage_ptr restrict stor, qn_svc_selector_ptr * restrict sel, qn_svc_type type)
{
    qn_service_ptr svc = qn_rgn_get_service(stor->rgn, type);
    if (! svc) svc = qn_svc_get_default_service(type);
    *sel = qn_svc_sel_create(svc, stor->sel.strategy, stor->sel.filter);
    return (*sel != NULL);
}

static inline qn_bool qn_stor_create_up_selector(qn_storage_ptr restrict stor)
{
    return qn_stor_create_service_selector(stor, &stor->sel.up, QN_SVC_UP);
}

static inline qn_bool qn_stor_create_io_selector(qn_storage_ptr restrict stor)
{
    return qn_stor_create_service_selector(stor, &stor->sel.io, QN_SVC_IO);
}

static inline qn_bool qn_stor_create_rs_selector(qn_storage_ptr restrict stor)
{
    return qn_stor_create_service_selector(stor, &stor->sel.rs, QN_SVC_RS);
}

static inline qn_bool qn_stor_create_rsf_selector(qn_storage_ptr restrict stor)
{
    return qn_stor_create_service_selector(stor, &stor->sel.rsf, QN_SVC_RSF);
}

QN_SDK void qn_stor_set_region(qn_storage_ptr restrict stor, qn_region_ptr restrict rgn, unsigned int strategy, unsigned int filter)
{
    assert(stor);
    if (stor->rgn) {
        qn_svc_sel_destroy(stor->sel.up);
        qn_svc_sel_destroy(stor->sel.io);
        qn_svc_sel_destroy(stor->sel.rs);
        qn_svc_sel_destroy(stor->sel.rsf);
        stor->sel.up = NULL;
        stor->sel.io = NULL;
        stor->sel.rs = NULL;
        stor->sel.rsf = NULL;
    ...
    stor->rgn = rgn;
    stor->sel.strategy = strategy;
    stor->sel.filter = filter;
}
...
static inline void qn_stor_reset(qn_storage_ptr restrict stor)
{
    ...
    if (stor->obj_body) {
        qn_json_obj_destroy(stor->obj_body);
        stor->obj_body = NULL;
    } // if
    if (stor->arr_body) {
        qn_json_arr_destroy(stor->arr_body);
        stor->arr_body = NULL;
    } // if
}
...
QN_SDK qn_json_object_ptr qn_stor_mn_api_stat(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict bucket, const char * restrict key, qn_stor_management_extra_ptr restrict mne)
{
    ...
    if (! ent) {
        if (! stor->sel.rs && ! qn_stor_create_rs_selector(stor)) return NULL;
        ent = qn_svc_sel_next_entry(stor->sel.rs);
    } // if
    ...
    url = qn_cs_sprintf("%s/%s", qn_str_cstr(ent->base_url), qn_str_cstr(op));
    qn_str_destroy(op);
    if (!url) return NULL;
    ...
    if (! qn_stor_mn_prepare(stor, url, ent->hostname, mac)) {
        qn_str_destroy(url);
        return NULL;
    } // if

    if (! (stor->obj_body = qn_json_obj_create())) {
        qn_str_destroy(url);
        return NULL;
    } // if

    if (! (qn_json_obj_set_integer(stor->obj_body, "fn-code", 0))) {
        qn_str_destroy(url);
        return NULL;
    } // if

    if (! (qn_json_obj_set_cstr(stor->obj_body, "fn-error", "OK"))) {
        qn_str_destroy(url);
        return NULL;
    } // if
...
QN_SDK qn_json_object_ptr qn_stor_mn_api_chgm(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict bucket, const char * restrict key, const char * restrict mime, qn_stor_management_extra_ptr restrict mne)
{
    ...
    url = qn_cs_sprintf("%s/chgm/%s/mime/%s", qn_str_cstr(ent->base_url), qn_str_cstr(encoded_uri), qn_str_cstr(encoded_mime));
    qn_str_destroy(encoded_uri);
...
QN_SDK qn_json_object_ptr qn_stor_bt_api_batch(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const qn_stor_batch_ptr restrict bt, qn_stor_management_extra_ptr restrict mne)
{
    ...
    url = qn_cs_sprintf("%s/batch", qn_str_cstr(ent->base_url));
    if (!url) {
        qn_str_destroy(body);
        return NULL;
    } // if
    ...
    if (! (fake_obj_body = qn_json_obj_create())) {
        qn_str_destroy(url);
        qn_str_destroy(body);
        return NULL;
    } // if

    if (! (qn_json_obj_set_integer(fake_obj_body, "fn-code", 0))) {
        qn_str_destroy(url);
        qn_str_destroy(body);
        return NULL;
    } // if

    if (! (qn_json_obj_set_cstr(fake_obj_body, "fn-error", "OK"))) {
        qn_str_destroy(url);
        qn_str_destroy(body);
        return NULL;
    } // if

    if (! (stor->arr_body = qn_json_obj_set_new_empty_array(fake_obj_body, "items"))) {
        qn_str_destroy(url);
        qn_str_destroy(body);
        return NULL;
    } // if
    ...
    if (stor->obj_body) {
        ...
        qn_json_obj_destroy(fake_obj_body);
    } else {
...
QN_SDK qn_json_object_ptr qn_stor_ls_api_list(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict bucket, qn_stor_list_extra_ptr restrict lse)
{
    ...
    if (! ent) {
        if (! stor->sel.rsf && ! qn_stor_create_rsf_selector(stor)) return NULL;
        ent = qn_svc_sel_next_entry(stor->sel.rsf);
    } // if
    ...
    // ---- Prepare the copy URL.
    url = qn_cs_sprintf("%s/list?%s", qn_str_cstr(ent->base_url), qn_str_cstr(qry_str));
...
QN_SDK void qn_stor_fte_set_service_entry(qn_stor_fetch_extra_ptr restrict fte, qn_svc_entry_ptr restrict ent)
{
    fte->ent = ent;
}
...
QN_SDK qn_json_object_ptr qn_stor_ft_api_fetch(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict src_url, const char * restrict dest_bucket, const char * restrict dest_key, qn_stor_fetch_extra_ptr restrict fte)
{
    ...
    if (! ent) {
        if (! stor->sel.io && ! qn_stor_create_io_selector(stor)) return NULL;
        ent = qn_svc_sel_next_entry(stor->sel.io);
    } // if
    ...
    url = qn_cs_sprintf("%s/fetch/%s/to/%s", qn_str_cstr(ent->base_url), qn_str_cstr(encoded_src_url), qn_str_cstr(encoded_dest_uri));
    qn_str_destroy(encoded_src_url);
    qn_str_destroy(encoded_dest_uri);
    if (!url) return NULL;
...
QN_SDK qn_json_object_ptr qn_stor_ft_api_prefetch(qn_storage_ptr restrict stor, const qn_mac_ptr restrict mac, const char * restrict dest_bucket, const char * restrict dest_key, qn_stor_fetch_extra_ptr restrict fte)
{
    ...
    url = qn_cs_sprintf("%.s/prefetch/%.s", qn_str_size(ent->base_url), qn_str_cstr(ent->base_url), qn_str_size(encoded_dest_uri), qn_str_cstr(encoded_dest_uri));
    qn_str_destroy(encoded_dest_uri);
    if (!url) return NULL;
...
{
    qn_json_object_ptr pp = qn_json_obj_create();
    if (pp) {
...
QN_SDK qn_bool qn_stor_pp_set_scope(qn_json_object_ptr restrict pp, const char * restrict bucket, const char * restrict key)
{
    ...
    if (key) {
        ...
        ret = qn_json_obj_set_cstr(pp, "scope", qn_str_cstr(scope));
        qn_str_destroy(scope);
    } else {
        ret = qn_json_obj_set_cstr(pp, "scope", bucket);
    } // if
    return ret;
}
...
{
    return qn_json_obj_set_integer(pp, "deadline", deadline);
}

QN_SDK qn_bool qn_stor_pp_dont_overwrite(qn_json_object_ptr restrict pp)
{
    return qn_json_obj_set_integer(pp, "insertOnly", 1);
}
...
{
    if (!qn_json_obj_set_cstr(pp, "returnUrl", url)) return qn_false;
    if (body) return qn_json_obj_set_cstr(pp, "returnBody", body);
    return qn_true;
}

QN_SDK qn_bool qn_stor_pp_return_to_client(qn_json_object_ptr restrict pp, const char * restrict body)
{
    return qn_json_obj_set_cstr(pp, "returnBody", body);
}
...
{
    if (!qn_json_obj_set_cstr(pp, "callbackUrl", url)) return qn_false;
    if (host_name) return qn_json_obj_set_cstr(pp, "callbackHost", host_name);
    return qn_true;
}
...
{
    if (!qn_json_obj_set_cstr(pp, "callbackBody", body)) return qn_false;
    if (mime_type) return qn_json_obj_set_cstr(pp, "callbackBodyType", mime_type);
    return qn_true;
}
...
QN_SDK qn_bool qn_stor_pp_pfop_set_commands(qn_json_object_ptr restrict pp, const char * restrict pipeline, const char * restrict cmd1, const char * restrict cmd2, ...)
{
    ...
    if (!cmd2) {
        ...
        ret = qn_json_obj_set_cstr(pp, "persistentOps", cmd1);
    } else {
        va_start(ap, cmd2);
...
QN_SDK qn_bool qn_stor_pp_pfop_set_command_list(qn_json_object_ptr restrict pp, const char * restrict pipeline, const char ** restrict cmds, qn_uint cmd_count)
{
    ...
    if (cmd_count == 1) {
        // Only one command passed.
        ret = qn_json_obj_set_cstr(pp, "persistentOps", cmds[0]);
    } else {
...
{
    return qn_json_obj_set_cstr(pp, "persistentNotifyUrl", url);
}

QN_SDK qn_bool qn_stor_pp_mime_enable_auto_detecting(qn_json_object_ptr restrict pp)
{
    return qn_json_obj_set_integer(pp, "detectMime", 1);
}
...
{
    if (! qn_json_obj_set_cstr(pp, "notifyQueue", msg_queue)) return qn_false;
    if (! qn_json_obj_set_cstr(pp, "notifyMessage", msg_body)) return qn_false;
    if (msg_mime_type) return qn_json_obj_set_cstr(pp, "notifyMessageType", msg_mime_type);
    return qn_true;
}
...
QN_SDK void qn_stor_upe_set_service_entry(qn_stor_upload_extra_ptr restrict upe, qn_svc_entry_ptr restrict ent)
{
    upe->ent = ent;
}
...
static qn_bool qn_stor_up_prepare_for_upload(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    // ----
    if (! (stor->obj_body = qn_json_obj_create())) return NULL;
    if (! (qn_json_obj_set_integer(stor->obj_body, "fn-code", 0))) return NULL;
    if (! (qn_json_obj_set_cstr(stor->obj_body, "fn-error", "OK"))) return NULL;
...
QN_SDK qn_json_object_ptr qn_stor_up_api_upload_file(qn_storage_ptr restrict stor, const char * restrict uptoken, const char * restrict fname, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    if (! ent) {
        if (! stor->sel.up && ! qn_stor_create_up_selector(stor)) return NULL;
        ent = qn_svc_sel_next_entry(stor->sel.up);
    } // if
...
static ssize_t qn_stor_ru_ctx_rdr_read_vfn(qn_io_reader_itf restrict itf, char * restrict buf, size_t buf_size)
{
    ...
    qn_json_object_ptr blk_info = NULL;
    qn_string ctx;
    ...
    while (rem_size > 0 && ru->ctx_idx < qn_json_arr_size(blk_arr)) {
        if (ru->need_comma) {
            *pos++ = ',';
            rem_size -= 1;
        } // if

        if (! qn_json_arr_get_object(blk_arr, ru->ctx_idx, &blk_info)) return -1;

        ctx = NULL;
        if (! qn_json_obj_get_string(blk_info, "ctx", &ctx)) return -1;
        if (! ctx) {
...
static qn_fsize qn_stor_ru_ctx_rdr_size_vfn(qn_io_reader_itf restrict itf)
{
    ...
    size = 0;
    for (i = 0; i < qn_json_arr_size(blk_arr); i += 1) {
        if (! qn_json_arr_get_object(blk_arr, i, &blk_info)) return 0;
        ctx = NULL;
        if (! qn_json_obj_get_string(blk_info, "ctx", &ctx)) return 0;
        if (! ctx) {
            qn_err_stor_set_lack_of_block_context();
            return 0;
        } // if
    ...
    size += qn_json_arr_size(blk_arr) - 1;
    return size;
}
...
QN_SDK qn_stor_resumable_upload_ptr qn_stor_ru_create(qn_io_reader_itf restrict data_rdr)
{
    ...
    ru->progress = qn_json_obj_create();
    if (! ru->progress) {
        free(ru);
        return NULL;
    } // if
    ...
    if (! qn_json_obj_set_integer(ru->progress, "bcount", ru->blk_cnt)) {
        qn_json_obj_destroy(ru->progress);
        free(ru);
        return NULL;
    } // if
    if (! qn_json_obj_set_new_empty_array(ru->progress, "blocks")) {
        qn_json_obj_destroy(ru->progress);
        free(ru);
        return NULL;
    } // if
...
QN_SDK qn_string qn_stor_ru_to_string(qn_stor_resumable_upload_ptr restrict ru)
{
    ...
    ret = qn_json_obj_set_text(ru->progress, "fsize", qn_str_cstr(fsize_str), qn_str_size(fsize_str));
    qn_str_destroy(fsize_str);
    if (! ret) return NULL;

    fsize_str = qn_type_fsize_to_string(ru->uploaded_fsize);
    ret = qn_json_obj_set_text(ru->progress, "uploaded_fsize", qn_str_cstr(fsize_str), qn_str_size(fsize_str));
    qn_str_destroy(fsize_str);
    if (! ret) return NULL;
...
QN_SDK qn_stor_resumable_upload_ptr qn_stor_ru_from_string(const char * restrict str, qn_size str_len)
{
    ...
    fsize_str = NULL;
    if (! qn_json_obj_get_string(ru->progress, "fsize", &fsize_str)) {
        qn_json_obj_destroy(ru->progress);
        free(ru);
        return NULL;
    }
    ...
    fsize_str = NULL;
    if (! qn_json_obj_get_string(ru->progress, "uploaded_fsize", &fsize_str)) {
        qn_json_obj_destroy(ru->progress);
        free(ru);
        return NULL;
    }
    ...
    cnt = 0;
    if (! qn_json_obj_get_integer(ru->progress, "bcount", &cnt)) {
        qn_json_obj_destroy(ru->progress);
        free(ru);
        return NULL;
    }
...
QN_SDK qn_json_object_ptr qn_stor_ru_get_block_info(qn_stor_resumable_upload_ptr restrict ru, int blk_idx)
{
    ...
    if (blk_idx > qn_json_arr_size(blk_arr)) {
        qn_err_set_out_of_range();
        return NULL;
    } // if
    if (blk_idx == QN_STOR_RU_BLOCK_LAST_INDEX) {
        blk_idx = qn_json_arr_size(blk_arr) - 1;
    } // if
    return (qn_json_arr_get_object(blk_arr, blk_idx, &blk_info)) ? blk_info : NULL;
}
...
QN_SDK qn_json_object_ptr qn_stor_ru_update_block_info(qn_stor_resumable_upload_ptr restrict ru, int blk_idx, qn_json_object_ptr restrict up_ret)
{
    ...
    crc32 = -1;
    if (! qn_json_obj_get_integer(up_ret, "crc32", &crc32)) return NULL;

    new_offset = -1;
    if (! qn_json_obj_get_integer(up_ret, "offset", &new_offset)) return NULL;
    ...
    new_blk_info = qn_json_obj_create();
    if (! new_blk_info) return NULL;

    if (! qn_json_obj_set_cstr(new_blk_info, "ctx", ctx)) goto QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING;
    if (! qn_json_obj_set_cstr(new_blk_info, "checksum", checksum)) goto QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING;
    if (! qn_json_obj_set_cstr(new_blk_info, "host", host)) goto QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING;
    if (! qn_json_obj_set_integer(new_blk_info, "crc32", crc32)) goto QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING;
    if (! qn_json_obj_set_integer(new_blk_info, "offset", new_offset)) goto QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING;
    if (! qn_json_obj_set_integer(new_blk_info, "bsize", blk_size)) goto QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING;

    blk_arr = NULL;
    if (! qn_json_obj_get_array(ru->progress, "blocks", &blk_arr)) return NULL;
    if (! qn_json_arr_replace_object(blk_arr, blk_idx, new_blk_info)) goto QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING;
    ru->uploaded_fsize += new_offset - old_offset;
    return new_blk_info;

QN_STOR_RU_UPDATE_BLOCK_INFO_ERROR_HANDLING:
    qn_json_obj_destroy(new_blk_info);
    return NULL;
}
...
QN_SDK qn_io_reader_itf qn_stor_ru_create_block_reader(qn_stor_resumable_upload_ptr restrict ru, int blk_idx, qn_json_object_ptr * restrict blk_info)
{
    ...
    if (ru->blk_cnt == 0) {
        ...
        if (! (new_blk_info = qn_json_arr_push_new_empty_object(blk_arr))) return NULL;
        if (! qn_json_obj_set_integer(new_blk_info, "bsize", blk_size)) return NULL;
    ...
    if (qn_json_arr_size(blk_arr) <= blk_idx) {
        for (i = qn_json_arr_size(blk_arr); i < blk_idx; i += 1) {
            if (! (new_blk_info = qn_json_arr_push_new_empty_object(blk_arr))) return NULL;
            if (! qn_json_obj_set_integer(new_blk_info, "bsize", blk_size)) return NULL;
        } // for
        if (i == ru->blk_cnt - 1) {
...
{
    qn_json_integer lhs = -1;
    qn_json_integer rhs = 0;
    // | offset | bsize | status    |
    // | -1     | > 0   | uploading |
    // | 0      | 0     | N/A       |
...
{
    if (! (qn_json_obj_set_integer(stor->obj_body, "fn-code", 0))) return qn_false;
    if (! (qn_json_obj_set_cstr(stor->obj_body, "fn-error", "OK"))) return qn_false;
    return qn_true;
}
...
static qn_bool qn_stor_ru_prepare_for_resumable_upload(qn_storage_ptr restrict stor, const char * restrict uptoken, const char * restrict mime, qn_io_reader_itf restrict rdr, qn_uint size, qn_svc_entry_ptr ent)
{
    qn_bool ret;
    qn_string tmp_hdr;
    ...
    if (! (stor->obj_body = qn_json_obj_create())) return qn_false;
...
QN_SDK qn_json_object_ptr qn_stor_ru_api_mkblk(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_io_reader_itf restrict data_rdr, qn_json_object_ptr restrict blk_info, qn_uint chk_size, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    if (! qn_stor_ru_prepare_for_resumable_upload(stor, uptoken, "application/octet-stream", data_rdr, chk_size, ent)) return NULL;

    // ---- Prepare upload URL.
    url = qn_cs_sprintf("%s/mkblk/%d", qn_str_cstr(ent->base_url), blk_size);
...
QN_SDK qn_json_object_ptr qn_stor_ru_api_bput(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_io_reader_itf restrict data_rdr, qn_json_object_ptr restrict blk_info, qn_uint chk_size, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    offset = -1;
    if (! qn_json_obj_get_integer(blk_info, "offset", &offset)) return NULL;
    if (offset <= 0) {
        qn_err_stor_set_lack_of_block_info();
        return NULL;
    } // if
    ...
    host = NULL;
    if (! qn_json_obj_get_string(blk_info, "host", &host)) return NULL;
    if (! host) {
        qn_err_stor_set_lack_of_block_info();
        return NULL;
    } // if
...
QN_SDK qn_json_object_ptr qn_stor_ru_api_mkfile(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_io_reader_itf restrict ctx_rdr, qn_json_object_ptr restrict last_blk_info, qn_fsize fsize, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    host = NULL;
    if (! qn_json_obj_get_string(last_blk_info, "host", &host)) return NULL;
    if (! host) {
        qn_err_stor_set_lack_of_block_info();
        return NULL;
    } // if
    ...
    if (! qn_stor_ru_prepare_for_resumable_upload(stor, uptoken, "text/plain", ctx_rdr, ctx_size, ent)) return NULL;
...
QN_SDK qn_json_object_ptr qn_stor_ru_upload_huge(qn_storage_ptr restrict stor, const char * restrict uptoken, qn_stor_resumable_upload_ptr ru, int * start_idx, qn_uint chk_size, qn_stor_upload_extra_ptr restrict upe)
{
    ...
    for (i = *start_idx; i < qn_stor_ru_get_block_count(ru); i += 1) {
        ...
        offset = 0;
        if (! qn_json_obj_get_integer(blk_info, "offset", &offset)) goto QN_STOR_UPLOAD_HUGE_ERROR_HANDLING;
        if (offset == 0) {
            ...
            if (! up_ret) goto QN_STOR_UPLOAD_HUGE_ERROR_HANDLING;

            code = -1;
            if (! qn_json_obj_get_integer(up_ret, "fn-code", &code) || code != 200) goto QN_STOR_UPLOAD_HUGE_ERROR_HANDLING;
